...
public class CustomerLoginController {
    ...
    // Lucide mail: M4 4h16c1.1 0 2 .9 2 2v12… + flap line
    private static final String MAIL_PATH =
        "M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z " +
        "M22 6l-10 7L2 6";
    // Lucide lock: body rect + shackle arc
    ...
    public void initialize() {
        // Header circle lock icon (larger — 30px visual in 64px circle)
        SVGPath headerLock = new SVGPath();
        headerLock.setContent(LOCK_PATH);
        headerLock.setStroke(Color.web("#1e293b"));
        headerLock.setFill(Color.TRANSPARENT);
        headerLock.setStrokeWidth(1.5);
        double hs = 30.0 / 24.0;
        headerLock.setScaleX(hs); headerLock.setScaleY(hs);
        headerIconLabel.setGraphic(headerLock);
    ...
    public void handleLogin(ActionEvent event) throws Exception {
        ...
        if (email.isEmpty() || password.isEmpty()) {
            ...
            return;
        } else if (!dbController.typechecker(email).equals("customer")) {
            showError("This is not customer account");
        } else if (dbController.loginchecker(email, password) == true && dbController.typechecker(email).equals("customer")) {

            // Save session
            UserSession.set(dbController.getUserName(email), email, "Customer");
